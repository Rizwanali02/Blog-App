...
const blogPost = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!req.files || Object.keys(req.files).length === 0) {
        return next(new ErrorHandle("Blog Main Image Is Must", 400))
    }

    const { mainImage, paraOneImage, paraTwoImage, paraThreeImage } = req.files;
    if (!mainImage) {
        return next(new ErrorHandle("Blog Main Image is Required", 400))
    }
...
const deleteBlog = catchAsyncErrors(async (req, res, next) => {
    //Deleteing Blog functionality
    const { id } = req.params;
    const blog = await Blog.findById(id);

    if (!blog) {
        return next(new ErrorHandle("Blog not found", 404))
    }
...
const updateBlogs = catchAsyncErrors(async (req, res, next) => {
